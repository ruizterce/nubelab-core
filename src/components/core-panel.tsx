...
import { sectionLabel, card, statusDot } from "./panel-styles";
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {group.services.map((svc, i) => (
                <div key={svc.name || `redacted-${i}`} style={card}>
                  <span style={statusDot} />
                  <div style={{ flex: 1 }}>
                    {svc.internal ? (
                      <span style={{ fontWeight: 600, color: "rgba(184, 92, 40, 0.6)" }}>[REDACTED]</span>
                    ) : svc.url ? (
                      <a href={svc.url} target="_blank" rel="noreferrer" style={{ color: "var(--accent)", textDecoration: "none", fontWeight: 600 }}>
                        {svc.name}
                      </a>
                    ) : (
                    ...
                    {!svc.internal && svc.desc && (
                      <span style={{ color: "var(--muted)", marginLeft: 8, fontSize: 12 }}>{svc.desc}</span>
                    )}
                    {svc.private && (
                  ...
                  <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.06em", color: "var(--accent-data)", textTransform: "uppercase" }}>
                    {svc.status}
                  </span>
                </div>
              ))}
